...
namespace mpt::arfunctors::core {
  ...
  public:
    ...
    template <class... Input> constexpr auto operator()(Input &&...arg) {
  ...
  template <class T>
  struct is_arfunctor
      : std::conditional_t<std::is_base_of_v<arfunctor, std::remove_cvref_t<T>>,
  ...
  namespace {
    template <class T> constexpr auto to_arfunctor(T &&obj) {
      using type = std::decay_t<T>;
      if constexpr (is_arfunctor_v<T>)
        return obj;
      else
        return make_arfunctor(std::forward<T>(obj));
    }
  } // namespace
  ...
  private:
    ...
    auto call_impl(std::index_sequence<I...>, Args &&...args) const {
      return operator_type{}(std::get<I>(m_operands)(args...)...);
    }
  };
