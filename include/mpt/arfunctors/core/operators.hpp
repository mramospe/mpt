  ...
  namespace {
    ...
    struct binary_operator_switcher {
      ...
      constexpr auto operator()(L &&lop, R &&rop) const {
        return make_composed_arfunctor<Operator>(std
                                                 : forward<L>(lop),
                                                   std::forward<R>(rop));
      }
    };
    ...
    struct binary_operator_switcher<Operator, runtime_arfunctor<Signature>,
      ...
      constexpr auto operator()(L &&lop, R &&rop) const {
        return make_runtime_composed_arfunctor<Operator>(
            std::forward<L>(lop), make_runtime_arfunctor<Signature>(rop));
      }
    };
    ...
    template <class Operator, class LeftOperand, class Signature>
    struct binary_operator_switcher<Operator, LeftOperand,
                                    runtime_arfunctor<Signature>> {
      template <class L, class R>
      constexpr auto operator()(L &&lop, R &&rop) const {
        return make_runtime_composed_arfunctor<Operator>(
            make_runtime_arfunctor<Signature>(lop), std::forward<L>(rop));
      }
    };
    ...
    struct binary_operator_switcher<Operator, runtime_arfunctor<Signature>,
      ...
      template <class L, class R>
      constexpr auto operator()(L &&lop, R &&rop) const {
        return make_runtime_composed_arfunctor<Operator>(std::forward<L>(lop),
                                                         std::forward<R>(rop));
      }
    };
    ...
    struct unary_operator_switcher<Operator, runtime_arfunctor<Signature>> {
      template <class T> constexpr auto operator()(T &&op) const {
        return make_runtime_composed_arfunctor<Operator>(std::forward<T>(op));
      }
    };
